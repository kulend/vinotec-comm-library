...
public class VinoLoadingAsyncTask<Params, Progress, Result> extends VinoAsyncTask<Params, Progress, Result> {
	...
	public VinoLoadingAsyncTask(VinoBasePage page) {
		this.mPage = page;
		if (page != null) {
        ...
        if(mTimeout > 0)
        {
            new Handler().postDelayed(new Runnable(){
                public void run() {
                    ...
                    if(!VinoLoadingAsyncTask.this.isCancelled() && InProcess)
                    {
                        VinoLoadingAsyncTask.this.cancel(false);
